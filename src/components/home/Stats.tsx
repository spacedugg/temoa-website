...
export function Stats({ tone = "blue" }: { tone?: "blue" | "white" }) {
  return (
    <section className={`section-y-sm relative ${tone === "blue" ? "bg-canvas-tint" : "bg-white"}`}>
      <div className="container-x">
        <RevealGroup className="grid grid-cols-2 gap-x-6 gap-y-10 md:grid-cols-4" stagger={0.08}>
          {stats.map((s) => (
            <RevealItem key={s.label}>
              <div className="text-center">
                <div className="text-4xl font-extrabold tracking-tight text-ink [font-variant-numeric:tabular-nums] md:text-5xl">
