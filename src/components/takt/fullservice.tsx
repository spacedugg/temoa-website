 ...
 */
/* Die Symbole der vier Schritte, in der Reihenfolge des Woerterbuchs. */
const ONBOARDING: IconName[] = ["kompass", "schild", "bericht", "ziel"];

export function Onboarding({ w }: { w: W["onboarding"] }) {
  const reduce = useReducedMotion();
  ...
  return (
    <Station label={w.label} tone="dark">
      ...
      <StationLead tone="dark">{w.lead}</StationLead>
          ...
          {w.schritte.map((o, i) => (
            ...
            >
              <span className="relative z-10 grid h-[4.1rem] w-[4.1rem] place-items-center rounded-[1.3rem] bg-[#0d2439] text-brand-500 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.1),0_0_0_6px_rgba(10,30,43,1)]">
                <Icon name={ONBOARDING[i]} className="h-8 w-8" />
              </span>
              ...
              <p className="mt-2.5 text-small leading-relaxed text-chalk-muted">{o.text}</p>
            </motion.div>
          ))}
        </div>
      </div>
